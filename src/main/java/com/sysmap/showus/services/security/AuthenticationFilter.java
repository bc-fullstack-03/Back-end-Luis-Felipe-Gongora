...
public class AuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(token == null || userId == null || !token.startsWith("Bearer ")){
            CustomErrors.sendError(response, HttpStatus.UNAUTHORIZED, "User not authenticated!", request);
            return;
        }
        ...
        try {
            ...
            CustomErrors.sendError(response, HttpStatus.UNAUTHORIZED, "Invalid token!", request);
            return;
        }
        if(isValidToken){
            try {
